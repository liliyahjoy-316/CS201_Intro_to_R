...
ggplot(data = penguins_data, aes(x = flipper_length_mm)) +
  geom_histogram(color = "lightblue", fill = "pink")
# what do we add here?
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.75) + # alpha chnaegd transparency
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Species") + #manual colors and name = will chaneg the table heading 
  theme_bw() + 
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.99) + # alpha chnaegd transparency
...
ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) + 
  ...
  theme_bw() + 
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Species") +
  labs(title = "Palmer Penguins' Flipper Length", 
       subtitle = "Three species observed on three islands in the Palmer Archipelago", 
       x = "Flipper Length (mm)", 
       y = "Number of Penguins") +
   theme(axes.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") +
  coord_flip()
...
```{r, message=FALSE}
culmen <- ggplot(data = penguins_data, aes(, x = culmen_depth_mm, y = body_mass_g, color = species, shape = island)) + 
  geom_point()  + 
  scale_color_viridis_d(option = "mako") + 
  scale_shape_manual(values = c(15,16,17)) + 
  labs(title = "Palmer Penguins' Culmen Depth", 
...
```{r}
 culmen_length <- ggplot(data = penguins_data, aes( x = culmen_length_mm, y = body_mass_g, color = species, shape = island)) + 
  geom_point()  + 

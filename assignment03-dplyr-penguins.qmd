...
```{r, messages=FALSE}
unique(penguins$island) # an example of how to use unique()
...
reloocate_penguins <- penguins |> 
  relocate(individual_id, .after = sample_number)

view(reloocate_penguins)
```
...
mutate <- penguins |> 
  mutate(flippometers = flipper_length_mm / 100)
...
number8 <- penguins |> 
  ...
  summarize(culmen_depth_mean = mean(culmen_depth_mm, na.rm = TRUE), culmen_length_mean = mean(culmen_length_mm, na.rm = TRUE))
...
case_when <- penguins |> 
  mutate(size = case_when(
    body_mass_g >= 4500 ~ "big",
    body_mass_g < 4500 ~ "small"
  ))
```
...
final_boss <- penguins |> 
  ...
  mutate(flipper_rank = case_when(flipper_length_mm < 190 ~ "short", flipper_length_mm >= 190 & flipper_length_mm <= 210  ~ "medium", flipper_length_mm > 210 ~ "long")) |> 
  group_by(species, island, flipper_rank) |> 
  summarize(body_mass_g= round(mean(body_mass_g, na.rm = TRUE)))
